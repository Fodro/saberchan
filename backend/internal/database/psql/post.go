...
func (r *repo) GetPosts(threadID uuid.UUID) ([]database.Post, error) {
	stmt := `SELECT id, number, text, sage, op_marker, ip, thread_id, created_at, browser_fingerprint FROM post WHERE thread_id = $1 ORDER BY number ASC`
	...
	for rows.Next() {
		var post database.Post
		if err := rows.Scan(&post.ID, &post.Number, &post.Text, &post.Sage, &post.OpMarker, &post.IP, &post.ThreadID, &post.CreatedAt, &post.BrowserFingerprint); err != nil {
			return nil, err
		}
...
func (r *repo) GetOPPost(threadID uuid.UUID) (*database.Post, error) {
	stmt := `SELECT id, number, text, thread_id, sage, op_marker, ip, created_at, browser_fingerprint FROM post WHERE thread_id = $1 ORDER BY number ASC LIMIT 1`
	row := r.db.QueryRow(stmt, threadID)
	var post database.Post
	if err := row.Scan(&post.ID, &post.Number, &post.Text, &post.ThreadID, &post.Sage, &post.OpMarker, &post.IP, &post.CreatedAt, &post.BrowserFingerprint); err != nil {
		return nil, err
	}
	return &post, nil
}
